...
- **Cómo llega el dato al navegador en vivo**: SSE, WebSocket o consulta periódica. No se decidió al construir la ingesta, y no por olvido: el modelo y el endpoint quedan listos para cualquiera de las tres, y elegir de taquito la que fuera más fácil de escribir ese día habría fijado por accidente cómo se comporta el producto con conexión intermitente, que es su escena principal. Se decide cuando exista el panel en tiempo real, con su porqué escrito. Hoy la pantalla lee al renderizar y declara la edad de cada dato, que es honesto y no se parece a estar en vivo.
