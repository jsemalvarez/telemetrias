...
El proceso del medio existe porque **Mosquitto no habla HTTP por sí solo**: es un broker MQTT puro, sin webhooks ni motor de reglas, a diferencia de EMQX o HiveMQ. Alguien tiene que suscribirse a los tópicos y hacer el pedido, y ese alguien es el puente. *Quedó construido el 2026-09-06, ver «El broker y el puente».* La aplicación no lo espera ni lo conoce: el contrato es HTTP y cualquier cosa que hable HTTP lo cumple, empezando por Postman.
...
**Cómo se actualiza el panel (2026-09-06).** Por **empuje**, con la consulta periódica de piso. Quedó construido el mismo día que se decidió.

*El empuje es Supabase Realtime Broadcast, con un tópico por empresa.* La ingesta avisa por el canal apenas guarda, y la pantalla se vuelve a pedir en ese instante. **El socket lo sostiene Supabase y no la aplicación**, que es lo que lo hace andar igual en una notebook que en Vercel — en serverless no hay proceso largo que pueda sostener una conexión abierta. Es también la razón por la que no hizo falta ni un servicio de sockets propio ni Redis: Supabase ya estaba en el stack por la base.

*El mensaje no lleva la medición: lleva el serial del equipo que habló.* Uno que llevara el valor tendría que llevar también la magnitud, su unidad, sus umbrales y su escala para poder dibujarse —un segundo formato de datos que mantener en sincronía— y dejaría el dibujo dependiendo de no haberse perdido ningún mensaje. Así el canal sólo dice «mirá de nuevo», y quien mira trae el estado entero.

*El corte por empresa sigue viviendo en la aplicación, y la política de Supabase lo comprueba.* El navegador se une con un token que emite la app **después** de correr `alcanzaCliente`, y ese token lleva un solo dato de dominio: la empresa. La política sobre el canal verifica que el portador no se cambie de tópico. Una decide, la otra comprueba — dos vueltas de llave sobre la misma puerta, y no dos criterios que algún día van a discrepar. **Publicar no está permitido desde el navegador**, a propósito: quien pudiera hacerlo inventaría lecturas que los demás verían como si vinieran de un equipo. Verificado con las dos empresas de la base: cada una recibe lo suyo, y el token de una contra el tópico de la otra no entra.

*Y el sondeo no se tira: baja a resincronización cada treinta segundos.* Deja de ser quien trae las novedades y pasa a ser la red que atrapa lo que el canal haya perdido mientras estuvo caído. Un canal que se cortó no sabe qué se perdió; una foto del estado completo no necesita saberlo. Sin canal —una instalación sin Supabase configurado— sube la cadencia y sostiene la pantalla solo: **la cadencia la pide el dato, no el reloj**, un segundo mientras algo se mueve y hasta cinco cuando la respuesta viene igual.

*La lámpara del panel dice cuál de los tres estados es*: «En vivo», «Sondeo» o «Sin enlace». Decir «en vivo» mientras en realidad se pregunta cada cinco segundos sería la clase de mentira chica que este producto no se permite.
...
**El broker y el puente (2026-09-06).** La cadena que estaba decidida quedó construida entera: el broker Mosquitto se levanta con los demás servicios y el proceso puente existe, se suscribe y reporta. La aplicación no cambió ni una línea por esto — el contrato siempre fue HTTP y sigue siéndolo.

*Cada equipo tiene su propia credencial contra el broker, y el usuario MQTT de un equipo es su serial.* De ahí sale, con dos líneas de ACL y sin ninguna lista que mantener, lo que la credencial del puente no puede dar: **un fierro no puede publicar en nombre de otro.** Son dos capas con dos alcances distintos y conviene no confundirlas — contra la aplicación se autentica el puente, una sola credencial de servicio que puede reportar por cualquier serial declarado; contra el broker se autentica cada equipo, y sólo puede hablar de lo suyo. Un equipo tampoco puede escuchar: publica y nada más. El puente es el espejo — escucha todo y no publica nada, igual que del otro lado escribe mediciones y no lee ninguna.

*El tópico se deriva del serial y no se guarda al lado.* Es la misma decisión que ya gobernaba la columna del serial: dos columnas que tienen que coincidir son dos columnas que algún día van a discrepar. El serial ya es único en todo el sistema y ya es lo que el equipo trae grabado.

*El puente agrupa por equipo antes de postear.* Un mensaje MQTT por magnitud no puede ser un POST por magnitud: la marca de tiempo la pone el puente —el microcontrolador no tiene reloj— y tres POST seguidos serían tres instantes distintos por milisegundos. En el panel eso se vería como tres agujas del mismo tablero con tres edades, y una siempre «la más vieja» sin que hubiera pasado nada. Lo que llega abajo de un mismo serial se junta durante un cuarto de segundo y sale junto, con un solo instante, sellado cuando llegó el primer mensaje del grupo.

*El puente no vuelve a juzgar lo que la aplicación ya juzga.* Una clave que nadie declaró, un valor que no es un número, un equipo dado de baja que sigue hablando: todo eso se manda igual y la respuesta dice qué pasó. Dos jueces sobre el mismo hecho es tener dos criterios que algún día no van a coincidir, y el que importa es el que escribe en la base. Lo que el puente sí hace es **registrar** lo que la respuesta le contesta, una vez por caso y no en cada vuelta: es la única forma de que alguien se entere de que hay un equipo instalado sin declarar, o una magnitud que se viene reportando y nadie escucha.

*Lo que se pierde y lo que no, dicho sin adornos.* El broker le guarda al puente lo que llegó mientras estuvo caído, **con la condición de que el equipo publique con QoS 1** — un mensaje QoS 0 no se encola para nadie que no esté escuchando, así está el protocolo y no hay configuración que lo cambie. Del lado del puente se pierde lo que estaba en la ventana durante una caída y lo que no entró después de los reintentos, y las dos cosas se escriben en el registro con todas las letras. Un reintento no duplica nada: viaja el mismo instante y choca contra la clave primaria de las mediciones.
...
- **Transporte de la telemetría** (2026-09-06): MQTT hasta un broker Mosquitto, y de ahí un proceso puente que reporta por REST a la aplicación. Contra la aplicación se autentica el puente, no cada equipo (ver «La ingesta de telemetría»). **Construido el mismo día**: el broker se levanta con los demás servicios, cada equipo tiene su credencial MQTT y el puente agrupa por equipo antes de postear (ver «El broker y el puente»).
- **Panel de lecturas** (2026-09-06): las mediciones se ven, por equipo y por magnitud, con instrumento de aguja donde hay escala declarada y con la edad de cada dato.
- **Actualización en tiempo real** (2026-09-06): por empuje, con Supabase Realtime Broadcast y un tópico por empresa; la consulta periódica queda de piso y de resincronización (ver «Cómo se actualiza el panel»).
...
- Detalle del despliegue. **La intención quedó declarada el 2026-09-06** y ya hay decisiones apoyadas en ella: la aplicación en Vercel, y el broker MQTT como un servicio aparte —Railway u otro similar— con el proceso puente reportando por REST. El proyecto de Supabase existe desde ese día (`monitoreo-iot`, São Paulo) y ya sostiene el canal en vivo; lo que falta es mudar la base y publicar la aplicación. Lo que esa forma ya decidió es cómo se actualiza el panel: serverless descarta un flujo abierto desde el servidor (ver «Cómo se actualiza el panel»).

**La aplicación corre donde está la base (2026-09-20).** El primer equipo real reportando tiró abajo la producción entera —no sólo la ingesta: también el acceso— con miles de `P2024`, «se acabó el tiempo esperando una conexión del pool». Dos cosas lo causaban, y las dos se arreglaron del lado del código, que es donde se pueden explicar: el pool dimensionado en 1 (ver `lib/db.ts`) y las funciones corriendo en Washington contra una base en São Paulo. Cada consulta cruzaba el continente, y con una sola conexión por instancia la segunda pedía turno hasta caerse. *El servicio va donde están los datos, y no al revés*: `vercel.json` fija `gru1`. La latencia entre la función y la base deja de ser un problema de diseño y pasa a ser un cable corto.
