...
type Admin = { usuario?: unknown; nombre?: unknown; clave?: unknown };
...
export async function POST(pedido: Request) {
  ...
  const [empresaTomada, usuarioTomado] = await Promise.all([
    ...
    db.user.findUnique({ where: { username: usuario }, select: { id: true } }),
  ]);
  ...
  if (usuarioTomado) {
    return error('usuario-repetido', `Ya hay un usuario «${usuario}».`, 409);
  }
  ...
  try {
    ...
    if (typeof falla === 'object' && falla && 'code' in falla && falla.code === 'P2002') {
      return error('repetido', 'Alguien acaba de dar de alta ese identificador o ese usuario.', 409);
    }
    throw falla;
  }
}
